 ...
 * Origin highlight.js/src/languages/lasso.js MD5 <d292a39014928955b542143a68fe0143>
 */
internal fun lasso(): Mode {
    var LASSO_IDENT_RE = "[a-zA-Z_][\\w.]*"
    var LASSO_ANGLE_RE = "<\\?(lasso(script)?|=)"
    var LASSO_CLOSE_RE = "\\]|\\?>"
    var LASSO_KEYWORDS = listOf(
        Keyword(
            className = "literal",
        ...
        Keyword(
            className = "built_in",
            value = "array date decimal duration integer map pair string tag xml null boolean bytes keyword list locale queue set stack staticarray local var variable global data self inherited currentcapture givenblock"
        ),
        Keyword(
            className = "keyword",
    ...
    var LASSO_START = Mode(
        className = "meta",
        begin = "\\[/noprocess|" + LASSO_ANGLE_RE
    )
    var LASSO_DATAMEMBER = Mode(
        className = "symbol",
        begin = "'\" + LASSO_IDENT_RE + \"'"
    )
    var LASSO_CODE = listOf(
