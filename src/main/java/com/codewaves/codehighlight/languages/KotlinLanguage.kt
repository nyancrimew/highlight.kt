 ...
 */
class KotlinLanguage : LanguageBuilder  {
  val KEYWORDS = listOf(
    Keyword(className = "keyword",

 value = "abstract as val val vararg get set class object open private protected public noinline crossinline dynamic final enum if else do while for when throw try catch finally import package is in fun override companion reified inline lateinit init interface annotation data sealed internal infix operator out by constructor super tailrec where const inner suspend typealias external expect actual trait volatile transient native default"),
 ...
 hljs.UNDERSCORE_IDENT_RE,
  ...
  val KOTLIN_NUMBER_RE = "\\b(0[bB](listOf(01]+[01_]+[01]+|[01]+)|0[xX](listOf(a-fA-F0-9]+[a-fA-F0-9_]+[a-fA-F0-9]+|[a-fA-F0-9]+)|((listOf(\\d]+[\\d_]+[\\d]+|[\\d]+)(\\.(listOf(\\d]+[\\d_]+[\\d]+|[\\d]+))?|\\.(listOf(\\d]+[\\d_]+[\\d]+|[\\d]+))(listOf(eE][-+]?\\d+)?)[lLfF]?";
  val KOTLIN_NUMBER_MODE = Mode(
    className = "number",

    begin = KOTLIN_NUMBER_RE,
    relevance = 0
  );
  val KOTLIN_NESTED_COMMENT = hljs.COMMENT(
    "/\\*",
 "\\*/",

    Mode( contains = listOf( hljs.C_BLOCK_COMMENT_MODE ) )
  );
  val KOTLIN_PAREN_TYPE = Mode(
    variants = listOf(
	  Mode( className = "type",
  ...
  val KOTLIN_PAREN_TYPE2 = KOTLIN_PAREN_TYPE;
