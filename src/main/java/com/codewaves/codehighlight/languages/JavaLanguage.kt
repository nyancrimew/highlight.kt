...
class JavaLanguage : LanguageBuilder  {
  ...
  val GENERIC_IDENT_RE = JAVA_IDENT_RE + "(<" +
 JAVA_IDENT_RE + "(\\s*,\\s*" +
 JAVA_IDENT_RE + ")*>)?";
  val KEYWORDS = "false synchronized int abstract float private char boolean val static null if const for true while long strictfp finally protected import native final void enum else break transient catch instanceof byte super volatile case assert short package default double public try this switch continue throws protected public private module requires exports do";

  // https://docs.oracle.com/javase/7/docs/technotes/guides/language/underscores-literals.html
  val JAVA_NUMBER_RE = "\\b(0[bB](listOf(01]+[01_]+[01]+|[01]+)|0[xX](listOf(a-fA-F0-9]+[a-fA-F0-9_]+[a-fA-F0-9]+|[a-fA-F0-9]+)|((listOf(\\d]+[\\d_]+[\\d]+|[\\d]+)(\\.(listOf(\\d]+[\\d_]+[\\d]+|[\\d]+))?|\\.(listOf(\\d]+[\\d_]+[\\d]+|[\\d]+))(listOf(eE][-+]?\\d+)?)[lLfF]?";
